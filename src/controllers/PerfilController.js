        ...
        execute(query, [cod_usuario_decoded])
            .then((result) => {
                if (result.length > 0) {
                    ...
                    res.status(200).send({nome : nome, username : username, email : email})
                    next()
                }
                else { res.status(404).send({ msg: `Usuario não existe` })}
            })
            .catch((error) => { res.status(500).send({ msg: error.sqlMessage }) })
    } 
    else { res.status(500).send({ msg: error.sqlMessage }) }
}
    ...
    if (nome && email && password) {
        execute("UPDATE USUARIOS SET nome = ?, email = ?, password ?, DATA_ULTIMA_ALTERACAO = NOW() WHERE cod_usuario = ?;", [nome, email, password, cod_usuario_decoded])
            .then((result) => {
                ...
                else { res.status(500).send({ msg: `Erro durante operação` }) }
            })
        ...
        execute("UPDATE USUARIOS SET nome = ?, email = ?, DATA_ULTIMA_ALTERACAO = NOW() WHERE cod_usuario = ?;", [nome, email, cod_usuario_decoded])
            .then((result) => {
                if (result.affectedRows > 0) {
                    res.status(200).send({ msg: `Informações de usuário editadas com sucesso` })
                    next()
                }
                else {
